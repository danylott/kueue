...
func TestWorkloadLabelFilter_Matches(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"matching labels returns true": {
			candidate: workload.NewInfo(utiltestingapi.MakeWorkload("wl", "ns").Label("tier", "preemptible").Obj()),
		...
		"non-matching label value returns false": {
			candidate: workload.NewInfo(utiltestingapi.MakeWorkload("wl", "ns").Label("tier", "guaranteed").Obj()),
			selector:  parseSelector(&metav1.LabelSelector{MatchLabels: map[string]string{"tier": "preemptible"}}),
			wantMatch: false,
		},
		"missing required label returns false": {
			candidate: workload.NewInfo(utiltestingapi.MakeWorkload("wl", "ns").Label("other", "value").Obj()),
			selector:  parseSelector(&metav1.LabelSelector{MatchLabels: map[string]string{"tier": "preemptible"}}),
			wantMatch: false,
		},
		...
		"matching matchExpressions returns true": {
			candidate: workload.NewInfo(utiltestingapi.MakeWorkload("wl", "ns").Label("tier", "dev").Obj()),
		...
		"non-matching matchExpressions returns false": {
			candidate: workload.NewInfo(utiltestingapi.MakeWorkload("wl", "ns").Label("tier", "prod").Obj()),
			selector: parseSelector(&metav1.LabelSelector{
				MatchExpressions: []metav1.LabelSelectorRequirement{
					{Key: "tier", Operator: metav1.LabelSelectorOpIn, Values: []string{"dev", "test"}},
				},
